...
RUN set -eux; \
    dnf5 install -y --setopt=install_weak_deps=False \
      ...
      cloud-utils-growpart python3-numpy kdialog; \
...
# ---- enable services -------------------------------------------------------------------------
RUN systemctl enable genesis-router.service genesis-ollama.service genesis-growpart.service genesis-probe.service genesis-firstrun.service genesis-router-refresh.service genesis-packs-refresh.service genesis-devssh.service genesis-bootc-status.service bootc-fetch-apply-updates.timer && systemctl --global enable genesis-crash.service genesis-power.service genesis-phone.service genesis-companiond.service genesis-packs.timer genesis-index.timer genesis-permd.service genesis-agentd.service \
